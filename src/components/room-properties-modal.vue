  ...
  <div>
    ...
    >
      <Form ref='roomForm' :model="roomInModal" :label-width="80">
        ...
        <FormItem v-if='renterList.length>0' label="住户">
          <Select v-model="roomInModal.renterIdList" filterable clearable placeholder='房间入住应从业务系统发起入住流程' multiple>
            <Option v-for="item in renterList" :value="item.id" :key="item.id">{{ item.name }}</Option>
          </Select>
        </FormItem>
            <FormItem label="费用模板" >
                <Transfer
...
export default class App extends Vue {
  ...
  public createRoom(roomGroupIdList) {
    ...
    this.roomInModal={roomGroupIdList:roomGroupIdList,renter:{},renterIdList : [],chargeTemplateIdList:[]}
    this.modalShow = true;
  }
